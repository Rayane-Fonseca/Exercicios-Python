class Pessoa:
    def __init__ (self, nome): #init: inicializar um objeto
        self.nome = nome
        self.parceiros = []

    def adicionar_parceiro(self, relacionamento):
        self.parceiros.append(relacionamento)

    def mostrar_parceiros(self):

        if not self.parceiros:
            print(f"{self.nome} não possui parceiros registrados")

        else: 
            print(f"Parceiros amorosos de {self.nome}:")
            for rel in self.parceiros:

                if rel.pessoa1 == self:
                    parceiro = rel.pessoa2
                else:
                    parceiro = rel.pessoa1
                print(f"{parceiro.nome}, desde {rel.ano_inicio}")

class Relacionamento:
    def __init__(self, pessoa1, pessoa2, ano_inicio):
        self.pessoa1 = pessoa1
        self.pessoa2 = pessoa2
        self.ano_inicio = ano_inicio

    def mostrar_informacao(self):
        print(f"{self.pessoa1} e {self.pesso2.nome} estão juntas desde {self.ano_inicio}")

#criando pessoas
ana = Pessoa("ana")
igona = Pessoa("Igona")
brino = Pessoa("Brino")
felca = Pessoa("Felca")
menina_veneno = Pessoa("Menina Veneno")
chico_moedas = Pessoa("Chico Moedas")
luisa = Pessoa("Luisa")
davi_brito = Pessoa("Davi Brito")

#criando relacionamento
relacionamento1 = Relacionamento(ana, igona, 2024)
relacionamento2 = Relacionamento(brino, luisa, 2025)
relacionamento3 = Relacionamento(menina_veneno, felca, 2021)
relacionamento4 = Relacionamento(chico_moedas, davi_brito, 2022)
relacionamento5 = Relacionamento(chico_moedas, luisa, 2023)

#associar relacionamento as pessoas
ana.adicionar_parceiro(relacionamento1)
igona.adicionar_parceiro(relacionamento1)

brino.adicionar_parceiro(relacionamento2)
luisa.adicionar_parceiro(relacionamento2)

menina_veneno.adicionar_parceiro(relacionamento3)
felca.adicionar_parceiro(relacionamento3)

chico_moedas.adicionar_parceiro(relacionamento4)
davi_brito.adicionar_parceiro(relacionamento4)

chico_moedas.adicionar_parceiro(relacionamento5)
luisa.adicionar_parceiro(relacionamento5)        

#exebir
chico_moedas.mostrar_parceiros()